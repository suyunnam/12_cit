...
First, let's specify:  
  - the **type of model** we want to train.  (e.g., conditional inference tree)
  - which **engine (package)** we want to use.  (e.g., partykit (p value is the standard) or party (r2 is the standard))
  - which **mode** we want to use. (e.g., regression for numerical x values)
...
```{r cit_spec}
cit_spec <- 
  # Specifying cit as our model type, asking to tune the tree-depth
 decision_tree(tree_depth = tune()) %>%
  # Specify the engine, asking to tune the p value threshold
 set_engine("partykit",
...
```
Why the 3 horizontal lines?  there are only three terminal nodes

How can we get more horizontal lines in this type of algorithm?  increase depth, flexibility, less restrict, more liberal
...
In the tree depth result was 10, but here final test set showed only 2 depth. It can happen because cit is susceptible in these.
...
In this exercise, we covered: 
  ...
  - Used **iterative search** (not the fixed grid) to find the best values for mas_depth and min_criterion    
